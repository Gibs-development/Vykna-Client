...
public class OrbsPanel extends PanelManager.BasePanel {
	...
	public void draw(Client client) {
		...
		client.drawOrbsAt(bounds.x, bounds.y + PanelManager.PANEL_HEADER_HEIGHT);
	...
	public boolean handleMouse(Client client, int mouseX, int mouseY) {
		...
		client.updateRs3OrbHovers(absoluteX, absoluteY, bounds.x, bounds.y + PanelManager.PANEL_HEADER_HEIGHT);
		client.processRs3OrbActions(absoluteX, absoluteY, bounds.x, bounds.y + PanelManager.PANEL_HEADER_HEIGHT);
		return true;
	}
	...
	public boolean handleClick(Client client, int mouseX, int mouseY) {
		...
		int absoluteX = bounds.x + mouseX;
		int absoluteY = bounds.y + mouseY;
		client.processRs3OrbClick(absoluteX, absoluteY, bounds.x, bounds.y + PanelManager.PANEL_HEADER_HEIGHT);
		return true;
	}
}
